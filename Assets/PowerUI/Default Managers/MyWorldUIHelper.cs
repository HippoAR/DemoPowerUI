...
namespace PowerUI{
	...
	public class MyWorldUIHelper : PowerUI.Manager { // It's a manager so it can share the same inspector properties

		[Header("Serf Props")]
		[SerializeField]
		private float customWidthInMeter; 
		[SerializeField]
		...
		void Update() {
			if (UnityEngine.Input.GetKeyDown (KeyCode.B)) {
				print ("press B");
				transform.localScale += new Vector3 (0f, 0f, 0.2f);
				WorldUI.UpdateResolution ();
			}
		}
	}
