...
namespace Loonim{
	...
	/// Note that these work best with the Stack node.
	/// The path ranges from +1 to -1 on x and y.
	/// </summary>
	
	public class ClippingPath: TextureNode{
		...
		/// <summary>The computed mesh to display. The path ranges from +1 to -1 on x and y.</summary>
		private Mesh Mesh_;
		#endif
		
		
		public ClippingPath():base(1){}
		
		public ClippingPath(TextureNode fill,Mesh mesh):base(1){
			SourceModule=fill;
			Mesh=mesh;
		}
		
		public ClippingPath(TextureNode fill,VectorPath path):base(1){
			SourceModule=fill;
			Path=path;
		}
