	...
	public static class DateParser
	{
		...
		private static DateTime ParseUnstructured(string input)
		{
			...
			for (int i = 0; i < words.Length; i++)
			{
				...
				else if (dayOfWeekNames.ContainsKey(word.ToLower()) == true)
				{
		...
		private static Dictionary<string,bool> dayOfWeekNames;

		/// <summary>
		/// Constructs a HashSet containing the names of days of the week.
		/// </summary>
		private static Dictionary<string,bool> PopulateDayOfWeekNames()
		{
			...
			foreach (string dayName in abbreviatedNames)
				result.Add(dayName.ToLower(),true);
