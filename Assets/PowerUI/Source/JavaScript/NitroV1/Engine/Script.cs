...
namespace Nitro{
	...
	public class Script{
		...
		public object RunLiteral(string functionName,Type[] typeSet,object[] args,bool optional){
			...
			#if !DisableVariableNitro
			// How many parameters does the method want?
			int requiredCount=mInfo.GetParameters().Length;
			
			if(args.Length!=requiredCount){
				
				// Resize args to fit:
				object[] newArgs=new object[requiredCount];
				
				int max=args.Length;
				
				if(max>requiredCount){
					max=requiredCount;
				}
