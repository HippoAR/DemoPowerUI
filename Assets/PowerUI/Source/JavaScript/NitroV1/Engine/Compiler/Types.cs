...
namespace Nitro{
	...
	public static class Types{
		...
		/// <summary>Converts the given set of parameters a set of types.</summary>
		/// <param name="set">The set of parameters to turn into a type set.</param>
		/// <returns>The set as an array of types.</returns>
		public static Type[] SetToTypes(ParameterInfo[] set){
			
			if(set==null){
				return null;
			}
			
			Type[] types=new Type[set.Length];
			
			for(int i=0;i<set.Length;i++){
				types[i]=set[i].ParameterType;
			}
		...
		/// <summary>Gets the MethodInfo from the given set of methods which matches the given argument set.</summary>
		...
		public static MethodInfo GetOverload(List<MethodInfo> overloads,Type[] argSet){
			// Try without then with casting:
			MethodInfo result=GetOverload(overloads,argSet,false);
			if(result!=null){
				return result;
			}
			// Not found, so try with casting:
			return GetOverload(overloads,argSet,true);
		}
		...
		/// <returns>The overload if found; null otherwise.</returns>
		private static MethodInfo GetOverload(List<MethodInfo> overloads,Type[] argSet,bool cast){
			if(overloads==null){
				return null;
			}
		...
		public static CompiledFragment TryCast(CompiledMethod method,CompiledFragment frag,Type to,Type from){
			bool isExplicit;
			return TryCast(method,frag,to,from,out isExplicit);
		}
		...
		/// <returns>A cast operation if it is possible; throws an error otherwise.</returns>
		public static CompiledFragment TryCast(CompiledMethod method,CompiledFragment frag,Type to,Type from,out bool isExplicit){
			isExplicit=false;
		...
		public static Type[] GetTypes(Type first,CompiledFragment[] frags){
			if(frags==null){
				return new Type[]{first};
			}
			
			Type[] result=new Type[frags.Length+1];
			
			result[0]=first;
			
			for(int i=frags.Length-1;i>=0;i--){
				result[i+1]=frags[i].OutputType();
			}
