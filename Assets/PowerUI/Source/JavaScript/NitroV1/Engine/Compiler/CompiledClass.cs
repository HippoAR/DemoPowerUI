...
namespace Nitro{
	...
	public class CompiledClass{
		...
		public void Compile(){
			...
			// Step 2. Collect anything else and place them in a function called OnScriptReady (or the constructor of classes).
			FindOperations(ClassFragment);
			...
			// Step 4. Compile the OnScriptReady function body.
			// This is because it contains all the globally scoped properties in traditional Nitro.
			// Therefore in order to 100% avoid runtime lookups, we do this first.
			MethodOverloads onScriptReady;
			
			if(Methods.TryGetValue("onscriptready",out onScriptReady)){
				
				// Compile it:
				onScriptReady.CompileBody();
				
			}
			
			// Step 5. Compile method bodies:
			foreach(KeyValuePair<string,MethodOverloads> kvp in Methods){
				
				if(kvp.Key=="onscriptready"){
					// Skip this one - compiled it above.
					continue;
				}
		...
		*/
		...
		internal virtual FieldInfo DefineField(string name,bool isPublic,Type type){
