...
namespace Nitro{
	...
	public class MethodOverloads{
		...
		public void CompileBody(){
			...
			MethodInfo scriptStartInfo=null;
			...
			if(tempMethod.Name=="new"){
				...
				// Same goes for onscriptready too:
				set=tempMethod.Parent.FindMethodSet("onscriptready");
				
				if(set!=null){
					scriptStartInfo=set.Methods[0].getMethodInfo();
				}
			...
			foreach(CompiledMethod method in Methods){
				...
				if(scriptStartInfo!=null){
					method.ILStream.Emit(OpCodes.Ldarg_0);
					method.ILStream.Emit(OpCodes.Call,scriptStartInfo);
				}
