...
namespace Nitro{
	...
	public class SetOperation:Operation{
		...
		public bool SelfReferencing(){
			...
			// Try getting it as a variable:
			Variable variable=Input0.Value as Variable;
			
			if(variable!=null){
				
				// Great it's a variable!
				return LookFor(Input1,variable);
				
			}else{
				
				// Try as a property instead.
				PropertyOperation property=Input0 as PropertyOperation;
				
				if(property!=null){
					
					// it's a property - try searching that instead!
					return LookFor(Input1,property);
