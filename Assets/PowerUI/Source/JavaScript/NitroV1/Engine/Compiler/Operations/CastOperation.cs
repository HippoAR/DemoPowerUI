...
namespace Nitro{
	...
	public class CastOperation:Operation{
		...
		public override Type OutputType(out CompiledFragment newOperation){
			...
			// Get the type being cast from:
			Type fromType=ToCast.OutputType(out ToCast);
			
			// Try casting it now:
			CompiledFragment result=Types.TryCast(Method,ToCast,ToType,fromType);
			
			if(result==null){
				Error("Cannot cast "+fromType+" to "+ToType+".");
			}
